            ...
            <div class="sendCrypto_step1_network-row">
              <button type="button" class="sendCrypto_step1_network-item" data-sim-crypto-network="ripple">
                <div class="sendCrypto_step1_icon-wrap">
                  ...
                  <div class="sendCrypto_step1_network-name-wrap">
                    <div class="sendCrypto_step1_network-name">Ripple Ledger</div>
                  </div>
                </div>
              </button>
...
></div>
  ...
  color: #1f1f1f;
    ...
    const sanitizeAmount = () => {
      let raw = (amountEl.textContent || "").trim();
      ...
      // Activate / deactivate trigger based on validity
      const hasValue = raw.length > 0;
      trigger.style.opacity = hasValue ? "1" : "0.4";
      trigger.style.pointerEvents = hasValue ? "auto" : "none";
    };

    // Run on input and blur
    amountEl.addEventListener("input", sanitizeAmount);
    amountEl.addEventListener("blur", sanitizeAmount);
